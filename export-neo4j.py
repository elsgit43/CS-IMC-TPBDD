...
#graph.run("DROP INDEX FOR (f:Film) ON (f.idFilm) IF EXISTS")
#graph.run("DROP INDEX FOR (a:Artist) ON (a.idArtist) IF EXISTS")
    ...
    while True:
        ...
        try:
            ...
            for cat in importData:
                ...
                # A COMPLETER
                for elem in importData[cat]:
                    print(elem)
                    create_relationships(graph.auto(),elem,cat.replace(" ","_"))
                i+=1
            exportedCount += len(rows)
